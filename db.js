...
async function get_past_messages() {
    let result = await dbclient.query("SELECT * FROM past_messages pm WHERE created > (NOW() - (pm.sessionlife * '1 hours'::interval))")
        .catch((err) => { console.error(err) })
    return result.rows
}

async function create_past_messages(msg_id, lifetime_in_h) {
    let querry = `INSERT INTO past_messages VALUES(${msg_id}, NOW(), ${lifetime_in_h})`
    console.log(querry)
